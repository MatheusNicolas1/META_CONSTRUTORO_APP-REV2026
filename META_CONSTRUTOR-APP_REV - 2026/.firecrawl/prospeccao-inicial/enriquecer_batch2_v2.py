...
Salva incrementalmente a cada 10 sites processados.
"""
import json, re, time, urllib.request, urllib.error, ssl, sys, os
from urllib.parse import urlparse

INPUT_FILE = os.path.join(os.path.dirname(__file__) or '.', 'sites_para_enriquecer.txt')
OUTPUT_FILE = os.path.join(os.path.dirname(__file__) or '.', 'enriquecidos_batch2.json')
STATE_FILE = os.path.join(os.path.dirname(__file__) or '.', 'enriquecidos_batch2_progress.json')
...
def main():
    ...
    log(f"Unique sites to process: {len(unique_urls)}")
    ...
    if os.path.exists(STATE_FILE):
        try:
            ...
            log(f"Resuming from previous progress: {len(results)} sites already processed")
        except:
            pass
    ...
    for i, url in enumerate(remaining, 1):
        ...
        if i % 5 == 0 or i == len(remaining):
            with open(STATE_FILE, 'w', encoding='utf-8') as f:
                json.dump({'results': results, 'last_index': len(results)}, f, ensure_ascii=False)
            with open(OUTPUT_FILE, 'w', encoding='utf-8') as f:
                json.dump(results, f, ensure_ascii=False, indent=2)
            log(f"  [Salvou progresso: {len(results)}/{len(unique_urls)}]")
    ...
    log(f"\n=== Resumo ===")
    log(f"Total sites únicos: {len(unique_urls)}")
    log(f"Sucesso (encontrou contato): {success_count}")
