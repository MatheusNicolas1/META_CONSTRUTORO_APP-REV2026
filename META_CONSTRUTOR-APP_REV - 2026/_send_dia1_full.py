#!/usr/bin/env python3
"""Send Dia 1 campaign to contacts 151-200 (indices 150-199). Single-file exec."""
import csv
import json
import time
...
log(f"Batch size (indices 150-199): {len(batch)}")
...
for i, r in enumerate(batch):
    ...
    if not email:
        log(f"  SKIP [{150+i}] {empresa}: no email")
        continue
    emails.append({"to": email, "nome_empresa": empresa})
    log(f"  OK   [{150+i}] {empresa} <{email}>")
...
# Summary
log(f"\n{'='*60}")
log(f"RESUMO DO ENVIO - Dia 1 - Contatos 151-200")
log(f"{'='*60}")
