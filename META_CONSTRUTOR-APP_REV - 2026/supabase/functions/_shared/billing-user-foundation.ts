...
const toOrgSlug = (name: string, userId: string) => {
    const base = name
        .normalize("NFD")
        .replace(/[\u0300-\u036f]/g, "")
        .toLowerCase()
...
const createReferralCode = (userId: string) =>
    `${userId.replace(/-/g, "").slice(0, 6)}${crypto.randomUUID().replace(/-/g, "").slice(0, 6)}`.slice(0, 12);

const findBillingOrgMember = async (supabaseAdmin: SupabaseAdminClient, userId: string) => {
...
const resolveUniqueProfileValue = async (
    supabaseAdmin: SupabaseAdminClient,
    column: "phone" | "cpf_cnpj",
    rawValue: unknown,
    userId: string,
) => {
...
const getProfile = async (supabaseAdmin: SupabaseAdminClient, userId: string) => {
    const { data, error } = await supabaseAdmin
        .from("profiles")
        .select("id, stripe_customer_id, email, name, company, phone, cpf_cnpj, plan_type, terms_accepted_at")
        .eq("id", userId)
        .maybeSingle();
...
const ensureProfile = async (
    ...
    const name = cleanText(checkoutProfile.name)
        ?? cleanText(existingProfile?.name)
    ...
    const phone = await resolveUniqueProfileValue(
        supabaseAdmin,
        "phone",
        cleanText(checkoutProfile.phone) ?? cleanText(existingProfile?.phone) ?? cleanText(metadata.phone),
        user.id,
    );
    const cpfCnpj = await resolveUniqueProfileValue(
        supabaseAdmin,
        "cpf_cnpj",
        cleanText(checkoutProfile.cpf_cnpj) ?? cleanText(existingProfile?.cpf_cnpj) ?? cleanText(metadata.cpf_cnpj),
        user.id,
    );
...
const ensureUserRole = async (supabaseAdmin: SupabaseAdminClient, userId: string) => {
    const { data, error } = await supabaseAdmin
        .from("user_roles")
        .select("id, role")
        .eq("user_id", userId)
...
const ensureUserSettings = async (supabaseAdmin: SupabaseAdminClient, userId: string) => {
    const { data, error } = await supabaseAdmin
        .from("user_settings")
        .select("id")
        .eq("user_id", userId)
...
const ensureUserCredits = async (supabaseAdmin: SupabaseAdminClient, userId: string) => {
    const { data, error } = await supabaseAdmin
        .from("user_credits")
        .select("id")
        .eq("user_id", userId)
    ...
    const { error: insertError } = await supabaseAdmin
        .from("user_credits")
        .insert({ user_id: userId, credits_balance: 7, plan_type: "free" });

    if (insertError && insertError.code !== "23505") throw insertError;
};

const ensureBillingOrgMember = async (
    supabaseAdmin: SupabaseAdminClient,
    userId: string,
...
const waitForBillingOrgMember = async (supabaseAdmin: SupabaseAdminClient, userId: string) => {
...
export const ensureBillingUserFoundation = async (
    supabaseAdmin: SupabaseAdminClient,
    user: BillingAuthUser,
    checkoutProfile: CheckoutProfileInput = {},
) => {
    ...
    const orgNameHint = cleanText(checkoutProfile.company)
        ?? cleanText(profile?.company)
        ?? cleanText(checkoutProfile.name)
        ?? cleanText(profile?.name)
        ?? cleanText(user.user_metadata?.name)
        ?? cleanText(user.email);

    const orgMember = await waitForBillingOrgMember(supabaseAdmin, user.id)
        ?? await ensureBillingOrgMember(supabaseAdmin, user.id, orgNameHint);
