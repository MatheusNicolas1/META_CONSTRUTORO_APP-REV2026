 ...
 */
async function ensureStripeCoupon(stripe: Stripe, coupon: any, resolvedPriceId: string): Promise<string> {
...
serve(async (req) => {
    ...
    try {
        ...
        const { priceId, plan, billing = "monthly", successUrl, cancelUrl, profile: checkoutProfile = {}, coupon_code } = await req.json();
        ...
        const { profile, orgMember } = await ensureBillingUserFoundation(supabaseAdmin, user, checkoutProfile);
        ...
        if (!customerId) {
            ...
            await saveStripeCustomerId(supabaseAdmin, user.id, customerId);
        }
        ...
        if (coupon_code && coupon_code.trim()) {
            ...
            stripeCouponId = await ensureStripeCoupon(stripe, appliedCoupon, resolvedPriceId);
