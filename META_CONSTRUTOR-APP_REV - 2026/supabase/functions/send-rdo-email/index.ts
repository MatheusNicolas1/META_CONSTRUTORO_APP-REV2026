...
serve(async (req) => {
  ...
  try {
    ...
    const payload: SendRDOEmailRequest = await req.json();
    ...
    const subject = payload.subject?.trim() || `RDO ${rdoNumber} - ${obraName}`;
    const userMessage = payload.motivo?.trim() || payload.message?.trim();

    const html = `
      <div style="font-family:Arial,sans-serif;color:#111827;line-height:1.5">
        <h2 style="margin:0 0 12px">RDO ${rdoNumber}</h2>
        <p><strong>Obra:</strong> ${obraName}</p>
        <p><strong>Data:</strong> ${rdo.data ?? "Nao informada"}</p>
        <p><strong>Status:</strong> ${rdo.status ?? "Nao informado"}</p>
        ${userMessage ? `<p><strong>Mensagem:</strong><br>${userMessage.replaceAll("\n", "<br>")}</p>` : ""}
